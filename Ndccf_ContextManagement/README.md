# Go API client for Ndccf_ContextManagement
...
```golang
import Ndccf_ContextManagement "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_ContextManagement.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_ContextManagement.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_ContextManagement.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ndccf_ContextManagement.ContextOperationServerVariables, map[string]map[string]string{
