# Go API client for Npcf_UEPolicyControl
...
```golang
ctx := context.WithValue(context.Background(), Npcf_UEPolicyControl.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_UEPolicyControl.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_UEPolicyControl.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Npcf_UEPolicyControl.ContextOperationServerVariables, map[string]map[string]string{
