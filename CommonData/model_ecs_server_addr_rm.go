/*
Common Data Types

Common Data Types for Service Based Interfaces.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved.   

API version: 1.4.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_CommonData

import (
	"encoding/json"
	"fmt"
)

// EcsServerAddrRm This data type is defined in the same way as the ' EcsServerAddr ' data type, but with the OpenAPI 'nullable: true' property. 
type EcsServerAddrRm struct {
	EcsServerAddr *EcsServerAddr
	NullValue *NullValue
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *EcsServerAddrRm) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into EcsServerAddr
	err = json.Unmarshal(data, &dst.EcsServerAddr);
	if err == nil {
		jsonEcsServerAddr, _ := json.Marshal(dst.EcsServerAddr)
		if string(jsonEcsServerAddr) == "{}" { // empty struct
			dst.EcsServerAddr = nil
		} else {
			return nil // data stored in dst.EcsServerAddr, return on the first match
		}
	} else {
		dst.EcsServerAddr = nil
	}

	// try to unmarshal JSON data into NullValue
	err = json.Unmarshal(data, &dst.NullValue);
	if err == nil {
		jsonNullValue, _ := json.Marshal(dst.NullValue)
		if string(jsonNullValue) == "{}" { // empty struct
			dst.NullValue = nil
		} else {
			return nil // data stored in dst.NullValue, return on the first match
		}
	} else {
		dst.NullValue = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(EcsServerAddrRm)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *EcsServerAddrRm) MarshalJSON() ([]byte, error) {
	if src.EcsServerAddr != nil {
		return json.Marshal(&src.EcsServerAddr)
	}

	if src.NullValue != nil {
		return json.Marshal(&src.NullValue)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableEcsServerAddrRm struct {
	value *EcsServerAddrRm
	isSet bool
}

func (v NullableEcsServerAddrRm) Get() *EcsServerAddrRm {
	return v.value
}

func (v *NullableEcsServerAddrRm) Set(val *EcsServerAddrRm) {
	v.value = val
	v.isSet = true
}

func (v NullableEcsServerAddrRm) IsSet() bool {
	return v.isSet
}

func (v *NullableEcsServerAddrRm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEcsServerAddrRm(val *EcsServerAddrRm) *NullableEcsServerAddrRm {
	return &NullableEcsServerAddrRm{value: val, isSet: true}
}

func (v NullableEcsServerAddrRm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEcsServerAddrRm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


