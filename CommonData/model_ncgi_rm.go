/*
Common Data Types

Common Data Types for Service Based Interfaces.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved.   

API version: 1.4.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package CommonData

import (
	"encoding/json"
	"fmt"
)

// NcgiRm This data type is defined in the same way as the 'Ncgi' data type, but with the  OpenAPI 'nullable: true' property.  
type NcgiRm struct {
	Ncgi *Ncgi
	NullValue *NullValue
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *NcgiRm) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into Ncgi
	err = json.Unmarshal(data, &dst.Ncgi);
	if err == nil {
		jsonNcgi, _ := json.Marshal(dst.Ncgi)
		if string(jsonNcgi) == "{}" { // empty struct
			dst.Ncgi = nil
		} else {
			return nil // data stored in dst.Ncgi, return on the first match
		}
	} else {
		dst.Ncgi = nil
	}

	// try to unmarshal JSON data into NullValue
	err = json.Unmarshal(data, &dst.NullValue);
	if err == nil {
		jsonNullValue, _ := json.Marshal(dst.NullValue)
		if string(jsonNullValue) == "{}" { // empty struct
			dst.NullValue = nil
		} else {
			return nil // data stored in dst.NullValue, return on the first match
		}
	} else {
		dst.NullValue = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(NcgiRm)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *NcgiRm) MarshalJSON() ([]byte, error) {
	if src.Ncgi != nil {
		return json.Marshal(&src.Ncgi)
	}

	if src.NullValue != nil {
		return json.Marshal(&src.NullValue)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableNcgiRm struct {
	value *NcgiRm
	isSet bool
}

func (v NullableNcgiRm) Get() *NcgiRm {
	return v.value
}

func (v *NullableNcgiRm) Set(val *NcgiRm) {
	v.value = val
	v.isSet = true
}

func (v NullableNcgiRm) IsSet() bool {
	return v.isSet
}

func (v *NullableNcgiRm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNcgiRm(val *NcgiRm) *NullableNcgiRm {
	return &NullableNcgiRm{value: val, isSet: true}
}

func (v NullableNcgiRm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNcgiRm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


