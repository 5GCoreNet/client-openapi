/*
Common Data Types

Common Data Types for Service Based Interfaces.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved.   

API version: 1.4.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package CommonData

import (
	"encoding/json"
	"fmt"
)

// HfcNodeIdRm This data type is defined in the same way as the 'HfcNodeId' data type, but with the OpenAPI 'nullable: true' property. 
type HfcNodeIdRm struct {
	HfcNodeId *HfcNodeId
	NullValue *NullValue
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *HfcNodeIdRm) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into HfcNodeId
	err = json.Unmarshal(data, &dst.HfcNodeId);
	if err == nil {
		jsonHfcNodeId, _ := json.Marshal(dst.HfcNodeId)
		if string(jsonHfcNodeId) == "{}" { // empty struct
			dst.HfcNodeId = nil
		} else {
			return nil // data stored in dst.HfcNodeId, return on the first match
		}
	} else {
		dst.HfcNodeId = nil
	}

	// try to unmarshal JSON data into NullValue
	err = json.Unmarshal(data, &dst.NullValue);
	if err == nil {
		jsonNullValue, _ := json.Marshal(dst.NullValue)
		if string(jsonNullValue) == "{}" { // empty struct
			dst.NullValue = nil
		} else {
			return nil // data stored in dst.NullValue, return on the first match
		}
	} else {
		dst.NullValue = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(HfcNodeIdRm)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *HfcNodeIdRm) MarshalJSON() ([]byte, error) {
	if src.HfcNodeId != nil {
		return json.Marshal(&src.HfcNodeId)
	}

	if src.NullValue != nil {
		return json.Marshal(&src.NullValue)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableHfcNodeIdRm struct {
	value *HfcNodeIdRm
	isSet bool
}

func (v NullableHfcNodeIdRm) Get() *HfcNodeIdRm {
	return v.value
}

func (v *NullableHfcNodeIdRm) Set(val *HfcNodeIdRm) {
	v.value = val
	v.isSet = true
}

func (v NullableHfcNodeIdRm) IsSet() bool {
	return v.isSet
}

func (v *NullableHfcNodeIdRm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableHfcNodeIdRm(val *HfcNodeIdRm) *NullableHfcNodeIdRm {
	return &NullableHfcNodeIdRm{value: val, isSet: true}
}

func (v NullableHfcNodeIdRm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableHfcNodeIdRm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


