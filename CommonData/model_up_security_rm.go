/*
Common Data Types

Common Data Types for Service Based Interfaces.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved.   

API version: 1.4.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package CommonData

import (
	"encoding/json"
	"fmt"
)

// UpSecurityRm This data type is defined in the same way as the 'UpSecurity' data type, but with the OpenAPI 'nullable: true' property. 
type UpSecurityRm struct {
	NullValue *NullValue
	UpSecurity *UpSecurity
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *UpSecurityRm) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into NullValue
	err = json.Unmarshal(data, &dst.NullValue);
	if err == nil {
		jsonNullValue, _ := json.Marshal(dst.NullValue)
		if string(jsonNullValue) == "{}" { // empty struct
			dst.NullValue = nil
		} else {
			return nil // data stored in dst.NullValue, return on the first match
		}
	} else {
		dst.NullValue = nil
	}

	// try to unmarshal JSON data into UpSecurity
	err = json.Unmarshal(data, &dst.UpSecurity);
	if err == nil {
		jsonUpSecurity, _ := json.Marshal(dst.UpSecurity)
		if string(jsonUpSecurity) == "{}" { // empty struct
			dst.UpSecurity = nil
		} else {
			return nil // data stored in dst.UpSecurity, return on the first match
		}
	} else {
		dst.UpSecurity = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(UpSecurityRm)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *UpSecurityRm) MarshalJSON() ([]byte, error) {
	if src.NullValue != nil {
		return json.Marshal(&src.NullValue)
	}

	if src.UpSecurity != nil {
		return json.Marshal(&src.UpSecurity)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableUpSecurityRm struct {
	value *UpSecurityRm
	isSet bool
}

func (v NullableUpSecurityRm) Get() *UpSecurityRm {
	return v.value
}

func (v *NullableUpSecurityRm) Set(val *UpSecurityRm) {
	v.value = val
	v.isSet = true
}

func (v NullableUpSecurityRm) IsSet() bool {
	return v.isSet
}

func (v *NullableUpSecurityRm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpSecurityRm(val *UpSecurityRm) *NullableUpSecurityRm {
	return &NullableUpSecurityRm{value: val, isSet: true}
}

func (v NullableUpSecurityRm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpSecurityRm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


