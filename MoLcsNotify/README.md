...
```golang
ctx := context.WithValue(context.Background(), MoLcsNotify.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), MoLcsNotify.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), MoLcsNotify.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), MoLcsNotify.ContextOperationServerVariables, map[string]map[string]string{
