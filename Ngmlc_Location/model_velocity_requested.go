/*
Ngmlc_Location

GMLC Location Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Ngmlc_Location

import (
	"encoding/json"
	"fmt"
)

// VelocityRequested Indicates velocity requirement.
type VelocityRequested struct {
	VelocityRequestedAnyOf *VelocityRequestedAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *VelocityRequested) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into VelocityRequestedAnyOf
	err = json.Unmarshal(data, &dst.VelocityRequestedAnyOf);
	if err == nil {
		jsonVelocityRequestedAnyOf, _ := json.Marshal(dst.VelocityRequestedAnyOf)
		if string(jsonVelocityRequestedAnyOf) == "{}" { // empty struct
			dst.VelocityRequestedAnyOf = nil
		} else {
			return nil // data stored in dst.VelocityRequestedAnyOf, return on the first match
		}
	} else {
		dst.VelocityRequestedAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(VelocityRequested)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *VelocityRequested) MarshalJSON() ([]byte, error) {
	if src.VelocityRequestedAnyOf != nil {
		return json.Marshal(&src.VelocityRequestedAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableVelocityRequested struct {
	value *VelocityRequested
	isSet bool
}

func (v NullableVelocityRequested) Get() *VelocityRequested {
	return v.value
}

func (v *NullableVelocityRequested) Set(val *VelocityRequested) {
	v.value = val
	v.isSet = true
}

func (v NullableVelocityRequested) IsSet() bool {
	return v.isSet
}

func (v *NullableVelocityRequested) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVelocityRequested(val *VelocityRequested) *NullableVelocityRequested {
	return &NullableVelocityRequested{value: val, isSet: true}
}

func (v NullableVelocityRequested) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVelocityRequested) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


