# Go API client for Ngmlc_Location
...
```golang
ctx := context.WithValue(context.Background(), Ngmlc_Location.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ngmlc_Location.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ngmlc_Location.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ngmlc_Location.ContextOperationServerVariables, map[string]map[string]string{
