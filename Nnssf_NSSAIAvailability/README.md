# Go API client for Nnssf_NSSAIAvailability
...
```golang
import Nnssf_NSSAIAvailability "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Nnssf_NSSAIAvailability.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nnssf_NSSAIAvailability.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nnssf_NSSAIAvailability.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nnssf_NSSAIAvailability.ContextOperationServerVariables, map[string]map[string]string{
