# Go API client for Nhss_imsUEAU
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUEAU.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUEAU.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUEAU.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nhss_imsUEAU.ContextOperationServerVariables, map[string]map[string]string{
