# Go API client for Exposure_Data
...
```golang
ctx := context.WithValue(context.Background(), Exposure_Data.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Exposure_Data.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Exposure_Data.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Exposure_Data.ContextOperationServerVariables, map[string]map[string]string{
