/*
3gpp-data-reporting

API for 3GPP Data Reporting.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package DataReporting

import (
	"encoding/json"
	"fmt"
)

// DataDomain A data reporting domain.
type DataDomain struct {
	DataDomainAnyOf *DataDomainAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *DataDomain) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into DataDomainAnyOf
	err = json.Unmarshal(data, &dst.DataDomainAnyOf);
	if err == nil {
		jsonDataDomainAnyOf, _ := json.Marshal(dst.DataDomainAnyOf)
		if string(jsonDataDomainAnyOf) == "{}" { // empty struct
			dst.DataDomainAnyOf = nil
		} else {
			return nil // data stored in dst.DataDomainAnyOf, return on the first match
		}
	} else {
		dst.DataDomainAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(DataDomain)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *DataDomain) MarshalJSON() ([]byte, error) {
	if src.DataDomainAnyOf != nil {
		return json.Marshal(&src.DataDomainAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableDataDomain struct {
	value *DataDomain
	isSet bool
}

func (v NullableDataDomain) Get() *DataDomain {
	return v.value
}

func (v *NullableDataDomain) Set(val *DataDomain) {
	v.value = val
	v.isSet = true
}

func (v NullableDataDomain) IsSet() bool {
	return v.isSet
}

func (v *NullableDataDomain) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDataDomain(val *DataDomain) *NullableDataDomain {
	return &NullableDataDomain{value: val, isSet: true}
}

func (v NullableDataDomain) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDataDomain) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


