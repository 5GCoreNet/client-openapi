/*
Namf_Communication

AMF Communication Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.2.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Namf_Communication

import (
	"encoding/json"
	"fmt"
)

// SmfChangeIndication Indicates the I-SMF or V-SMF change or removal
type SmfChangeIndication struct {
	SmfChangeIndicationAnyOf *SmfChangeIndicationAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *SmfChangeIndication) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into SmfChangeIndicationAnyOf
	err = json.Unmarshal(data, &dst.SmfChangeIndicationAnyOf);
	if err == nil {
		jsonSmfChangeIndicationAnyOf, _ := json.Marshal(dst.SmfChangeIndicationAnyOf)
		if string(jsonSmfChangeIndicationAnyOf) == "{}" { // empty struct
			dst.SmfChangeIndicationAnyOf = nil
		} else {
			return nil // data stored in dst.SmfChangeIndicationAnyOf, return on the first match
		}
	} else {
		dst.SmfChangeIndicationAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(SmfChangeIndication)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *SmfChangeIndication) MarshalJSON() ([]byte, error) {
	if src.SmfChangeIndicationAnyOf != nil {
		return json.Marshal(&src.SmfChangeIndicationAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableSmfChangeIndication struct {
	value *SmfChangeIndication
	isSet bool
}

func (v NullableSmfChangeIndication) Get() *SmfChangeIndication {
	return v.value
}

func (v *NullableSmfChangeIndication) Set(val *SmfChangeIndication) {
	v.value = val
	v.isSet = true
}

func (v NullableSmfChangeIndication) IsSet() bool {
	return v.isSet
}

func (v *NullableSmfChangeIndication) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSmfChangeIndication(val *SmfChangeIndication) *NullableSmfChangeIndication {
	return &NullableSmfChangeIndication{value: val, isSet: true}
}

func (v NullableSmfChangeIndication) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSmfChangeIndication) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


