...
```golang
ctx := context.WithValue(context.Background(), Nudm_PP.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nudm_PP.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nudm_PP.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nudm_PP.ContextOperationServerVariables, map[string]map[string]string{
