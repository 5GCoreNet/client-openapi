# Go API client for DataReportingProvisioning
...
```golang
import DataReportingProvisioning "//"
```
...
```golang
ctx := context.WithValue(context.Background(), DataReportingProvisioning.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), DataReportingProvisioning.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), DataReportingProvisioning.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), DataReportingProvisioning.ContextOperationServerVariables, map[string]map[string]string{
