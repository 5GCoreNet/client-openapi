# Go API client for openapi_AiMlNrm
...
```golang
import openapi_AiMlNrm "///openapi_AiMlNrm"
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_AiMlNrm.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_AiMlNrm.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_AiMlNrm.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), openapi_AiMlNrm.ContextOperationServerVariables, map[string]map[string]string{
