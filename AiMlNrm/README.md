...
```golang
ctx := context.WithValue(context.Background(), AiMlNrm.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), AiMlNrm.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), AiMlNrm.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), AiMlNrm.ContextOperationServerVariables, map[string]map[string]string{
