# Go API client for Nnef_EASDeployment
...
```golang
ctx := context.WithValue(context.Background(), Nnef_EASDeployment.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nnef_EASDeployment.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nnef_EASDeployment.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nnef_EASDeployment.ContextOperationServerVariables, map[string]map[string]string{
...
## Documentation For Models
 ...
 - [EasEventAnyOf](docs/EasEventAnyOf.md)
