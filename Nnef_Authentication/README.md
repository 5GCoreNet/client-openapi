# Go API client for Nnef_Authentication
...
```golang
ctx := context.WithValue(context.Background(), Nnef_Authentication.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nnef_Authentication.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nnef_Authentication.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nnef_Authentication.ContextOperationServerVariables, map[string]map[string]string{
