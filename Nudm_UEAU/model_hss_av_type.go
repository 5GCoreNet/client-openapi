...
func (dst *HssAvType) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into HssAvTypeAnyOf
	err = json.Unmarshal(data, &dst.HssAvTypeAnyOf);
	if err == nil {
		jsonHssAvTypeAnyOf, _ := json.Marshal(dst.HssAvTypeAnyOf)
		if string(jsonHssAvTypeAnyOf) == "{}" { // empty struct
			dst.HssAvTypeAnyOf = nil
		} else {
			return nil // data stored in dst.HssAvTypeAnyOf, return on the first match
		}
	} else {
		dst.HssAvTypeAnyOf = nil
	}
...
func (src *HssAvType) MarshalJSON() ([]byte, error) {
	if src.HssAvTypeAnyOf != nil {
		return json.Marshal(&src.HssAvTypeAnyOf)
	}
