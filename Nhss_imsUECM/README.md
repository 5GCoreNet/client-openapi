# Go API client for Nhss_imsUECM
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUECM.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUECM.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nhss_imsUECM.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nhss_imsUECM.ContextOperationServerVariables, map[string]map[string]string{
