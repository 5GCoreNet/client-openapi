# Go API client for Nmbsf_MBSUserDataIngestSession
...
```golang
import Nmbsf_MBSUserDataIngestSession "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Nmbsf_MBSUserDataIngestSession.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nmbsf_MBSUserDataIngestSession.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nmbsf_MBSUserDataIngestSession.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nmbsf_MBSUserDataIngestSession.ContextOperationServerVariables, map[string]map[string]string{
