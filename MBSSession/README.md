...
```golang
ctx := context.WithValue(context.Background(), MBSSession.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), MBSSession.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), MBSSession.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), MBSSession.ContextOperationServerVariables, map[string]map[string]string{
