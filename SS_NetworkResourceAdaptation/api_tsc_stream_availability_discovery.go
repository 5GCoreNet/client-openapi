...
type TSCStreamAvailabilityDiscoveryApi interface {
	...
	*/
	GetTscStreamAvailability(ctx context.Context) ApiGetTscStreamAvailabilityRequest

	// GetTscStreamAvailabilityExecute executes the request
	//  @return []TscStreamAvailability
	GetTscStreamAvailabilityExecute(r ApiGetTscStreamAvailabilityRequest) ([]TscStreamAvailability, *http.Response, error)
}
...
type ApiGetTscStreamAvailabilityRequest struct {
	ctx context.Context
	ApiService TSCStreamAvailabilityDiscoveryApi
...
func (r ApiGetTscStreamAvailabilityRequest) Execute() ([]TscStreamAvailability, *http.Response, error) {
	return r.ApiService.GetTscStreamAvailabilityExecute(r)
}
...
func (a *TSCStreamAvailabilityDiscoveryApiService) GetTscStreamAvailabilityExecute(r ApiGetTscStreamAvailabilityRequest) ([]TscStreamAvailability, *http.Response, error) {
	...
	localVarPath := localBasePath + "/tsc-stream-availability"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.streamSpecs == nil {
		return localVarReturnValue, nil, reportError("streamSpecs is required and must be specified")
	}
	if len(*r.streamSpecs) < 1 {
		return localVarReturnValue, nil, reportError("streamSpecs must have at least 1 elements")
	}

	{
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("stream-specs", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("stream-specs", parameterToString(t, "multi"))
		}
	}
