...
```golang
ctx := context.WithValue(context.Background(), SS_Events.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), SS_Events.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), SS_Events.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), SS_Events.ContextOperationServerVariables, map[string]map[string]string{
