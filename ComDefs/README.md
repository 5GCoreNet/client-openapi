...
```golang
ctx := context.WithValue(context.Background(), ComDefs.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), ComDefs.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), ComDefs.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), ComDefs.ContextOperationServerVariables, map[string]map[string]string{
