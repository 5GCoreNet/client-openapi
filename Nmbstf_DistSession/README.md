# Go API client for Nmbstf_DistSession
...
```golang
ctx := context.WithValue(context.Background(), Nmbstf_DistSession.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nmbstf_DistSession.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nmbstf_DistSession.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nmbstf_DistSession.ContextOperationServerVariables, map[string]map[string]string{
