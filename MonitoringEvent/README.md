# Go API client for MonitoringEvent
...
```golang
ctx := context.WithValue(context.Background(), MonitoringEvent.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), MonitoringEvent.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), MonitoringEvent.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), MonitoringEvent.ContextOperationServerVariables, map[string]map[string]string{
