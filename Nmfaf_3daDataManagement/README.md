# Go API client for Nmfaf_3daDataManagement
...
```golang
import Nmfaf_3daDataManagement "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Nmfaf_3daDataManagement.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nmfaf_3daDataManagement.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nmfaf_3daDataManagement.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nmfaf_3daDataManagement.ContextOperationServerVariables, map[string]map[string]string{
