/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// IMSSessionPriority struct for IMSSessionPriority
type IMSSessionPriority struct {
	IMSSessionPriorityAnyOf *IMSSessionPriorityAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *IMSSessionPriority) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into IMSSessionPriorityAnyOf
	err = json.Unmarshal(data, &dst.IMSSessionPriorityAnyOf);
	if err == nil {
		jsonIMSSessionPriorityAnyOf, _ := json.Marshal(dst.IMSSessionPriorityAnyOf)
		if string(jsonIMSSessionPriorityAnyOf) == "{}" { // empty struct
			dst.IMSSessionPriorityAnyOf = nil
		} else {
			return nil // data stored in dst.IMSSessionPriorityAnyOf, return on the first match
		}
	} else {
		dst.IMSSessionPriorityAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(IMSSessionPriority)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *IMSSessionPriority) MarshalJSON() ([]byte, error) {
	if src.IMSSessionPriorityAnyOf != nil {
		return json.Marshal(&src.IMSSessionPriorityAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableIMSSessionPriority struct {
	value *IMSSessionPriority
	isSet bool
}

func (v NullableIMSSessionPriority) Get() *IMSSessionPriority {
	return v.value
}

func (v *NullableIMSSessionPriority) Set(val *IMSSessionPriority) {
	v.value = val
	v.isSet = true
}

func (v NullableIMSSessionPriority) IsSet() bool {
	return v.isSet
}

func (v *NullableIMSSessionPriority) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableIMSSessionPriority(val *IMSSessionPriority) *NullableIMSSessionPriority {
	return &NullableIMSSessionPriority{value: val, isSet: true}
}

func (v NullableIMSSessionPriority) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableIMSSessionPriority) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


