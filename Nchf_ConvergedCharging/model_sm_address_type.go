/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// SMAddressType struct for SMAddressType
type SMAddressType struct {
	SMAddressTypeAnyOf *SMAddressTypeAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *SMAddressType) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into SMAddressTypeAnyOf
	err = json.Unmarshal(data, &dst.SMAddressTypeAnyOf);
	if err == nil {
		jsonSMAddressTypeAnyOf, _ := json.Marshal(dst.SMAddressTypeAnyOf)
		if string(jsonSMAddressTypeAnyOf) == "{}" { // empty struct
			dst.SMAddressTypeAnyOf = nil
		} else {
			return nil // data stored in dst.SMAddressTypeAnyOf, return on the first match
		}
	} else {
		dst.SMAddressTypeAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(SMAddressType)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *SMAddressType) MarshalJSON() ([]byte, error) {
	if src.SMAddressTypeAnyOf != nil {
		return json.Marshal(&src.SMAddressTypeAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableSMAddressType struct {
	value *SMAddressType
	isSet bool
}

func (v NullableSMAddressType) Get() *SMAddressType {
	return v.value
}

func (v *NullableSMAddressType) Set(val *SMAddressType) {
	v.value = val
	v.isSet = true
}

func (v NullableSMAddressType) IsSet() bool {
	return v.isSet
}

func (v *NullableSMAddressType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSMAddressType(val *SMAddressType) *NullableSMAddressType {
	return &NullableSMAddressType{value: val, isSet: true}
}

func (v NullableSMAddressType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSMAddressType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


