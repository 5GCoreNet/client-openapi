/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// DeliveryReportRequested struct for DeliveryReportRequested
type DeliveryReportRequested struct {
	DeliveryReportRequestedAnyOf *DeliveryReportRequestedAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *DeliveryReportRequested) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into DeliveryReportRequestedAnyOf
	err = json.Unmarshal(data, &dst.DeliveryReportRequestedAnyOf);
	if err == nil {
		jsonDeliveryReportRequestedAnyOf, _ := json.Marshal(dst.DeliveryReportRequestedAnyOf)
		if string(jsonDeliveryReportRequestedAnyOf) == "{}" { // empty struct
			dst.DeliveryReportRequestedAnyOf = nil
		} else {
			return nil // data stored in dst.DeliveryReportRequestedAnyOf, return on the first match
		}
	} else {
		dst.DeliveryReportRequestedAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(DeliveryReportRequested)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *DeliveryReportRequested) MarshalJSON() ([]byte, error) {
	if src.DeliveryReportRequestedAnyOf != nil {
		return json.Marshal(&src.DeliveryReportRequestedAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableDeliveryReportRequested struct {
	value *DeliveryReportRequested
	isSet bool
}

func (v NullableDeliveryReportRequested) Get() *DeliveryReportRequested {
	return v.value
}

func (v *NullableDeliveryReportRequested) Set(val *DeliveryReportRequested) {
	v.value = val
	v.isSet = true
}

func (v NullableDeliveryReportRequested) IsSet() bool {
	return v.isSet
}

func (v *NullableDeliveryReportRequested) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDeliveryReportRequested(val *DeliveryReportRequested) *NullableDeliveryReportRequested {
	return &NullableDeliveryReportRequested{value: val, isSet: true}
}

func (v NullableDeliveryReportRequested) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDeliveryReportRequested) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


