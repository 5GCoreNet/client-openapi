/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// ReplyPathRequested struct for ReplyPathRequested
type ReplyPathRequested struct {
	ReplyPathRequestedAnyOf *ReplyPathRequestedAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *ReplyPathRequested) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into ReplyPathRequestedAnyOf
	err = json.Unmarshal(data, &dst.ReplyPathRequestedAnyOf);
	if err == nil {
		jsonReplyPathRequestedAnyOf, _ := json.Marshal(dst.ReplyPathRequestedAnyOf)
		if string(jsonReplyPathRequestedAnyOf) == "{}" { // empty struct
			dst.ReplyPathRequestedAnyOf = nil
		} else {
			return nil // data stored in dst.ReplyPathRequestedAnyOf, return on the first match
		}
	} else {
		dst.ReplyPathRequestedAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(ReplyPathRequested)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *ReplyPathRequested) MarshalJSON() ([]byte, error) {
	if src.ReplyPathRequestedAnyOf != nil {
		return json.Marshal(&src.ReplyPathRequestedAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableReplyPathRequested struct {
	value *ReplyPathRequested
	isSet bool
}

func (v NullableReplyPathRequested) Get() *ReplyPathRequested {
	return v.value
}

func (v *NullableReplyPathRequested) Set(val *ReplyPathRequested) {
	v.value = val
	v.isSet = true
}

func (v NullableReplyPathRequested) IsSet() bool {
	return v.isSet
}

func (v *NullableReplyPathRequested) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReplyPathRequested(val *ReplyPathRequested) *NullableReplyPathRequested {
	return &NullableReplyPathRequested{value: val, isSet: true}
}

func (v NullableReplyPathRequested) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReplyPathRequested) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


