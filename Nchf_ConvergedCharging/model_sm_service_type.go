/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// SMServiceType struct for SMServiceType
type SMServiceType struct {
	SMServiceTypeAnyOf *SMServiceTypeAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *SMServiceType) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into SMServiceTypeAnyOf
	err = json.Unmarshal(data, &dst.SMServiceTypeAnyOf);
	if err == nil {
		jsonSMServiceTypeAnyOf, _ := json.Marshal(dst.SMServiceTypeAnyOf)
		if string(jsonSMServiceTypeAnyOf) == "{}" { // empty struct
			dst.SMServiceTypeAnyOf = nil
		} else {
			return nil // data stored in dst.SMServiceTypeAnyOf, return on the first match
		}
	} else {
		dst.SMServiceTypeAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(SMServiceType)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *SMServiceType) MarshalJSON() ([]byte, error) {
	if src.SMServiceTypeAnyOf != nil {
		return json.Marshal(&src.SMServiceTypeAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableSMServiceType struct {
	value *SMServiceType
	isSet bool
}

func (v NullableSMServiceType) Get() *SMServiceType {
	return v.value
}

func (v *NullableSMServiceType) Set(val *SMServiceType) {
	v.value = val
	v.isSet = true
}

func (v NullableSMServiceType) IsSet() bool {
	return v.isSet
}

func (v *NullableSMServiceType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSMServiceType(val *SMServiceType) *NullableSMServiceType {
	return &NullableSMServiceType{value: val, isSet: true}
}

func (v NullableSMServiceType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSMServiceType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


