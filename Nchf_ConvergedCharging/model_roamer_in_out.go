/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// RoamerInOut struct for RoamerInOut
type RoamerInOut struct {
	RoamerInOutAnyOf *RoamerInOutAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *RoamerInOut) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into RoamerInOutAnyOf
	err = json.Unmarshal(data, &dst.RoamerInOutAnyOf);
	if err == nil {
		jsonRoamerInOutAnyOf, _ := json.Marshal(dst.RoamerInOutAnyOf)
		if string(jsonRoamerInOutAnyOf) == "{}" { // empty struct
			dst.RoamerInOutAnyOf = nil
		} else {
			return nil // data stored in dst.RoamerInOutAnyOf, return on the first match
		}
	} else {
		dst.RoamerInOutAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(RoamerInOut)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *RoamerInOut) MarshalJSON() ([]byte, error) {
	if src.RoamerInOutAnyOf != nil {
		return json.Marshal(&src.RoamerInOutAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableRoamerInOut struct {
	value *RoamerInOut
	isSet bool
}

func (v NullableRoamerInOut) Get() *RoamerInOut {
	return v.value
}

func (v *NullableRoamerInOut) Set(val *RoamerInOut) {
	v.value = val
	v.isSet = true
}

func (v NullableRoamerInOut) IsSet() bool {
	return v.isSet
}

func (v *NullableRoamerInOut) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRoamerInOut(val *RoamerInOut) *NullableRoamerInOut {
	return &NullableRoamerInOut{value: val, isSet: true}
}

func (v NullableRoamerInOut) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRoamerInOut) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


