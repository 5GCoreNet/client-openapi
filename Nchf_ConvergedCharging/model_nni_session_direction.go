/*
Nchf_ConvergedCharging

ConvergedCharging Service    © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved. 

API version: 3.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nchf_ConvergedCharging

import (
	"encoding/json"
	"fmt"
)

// NNISessionDirection struct for NNISessionDirection
type NNISessionDirection struct {
	NNISessionDirectionAnyOf *NNISessionDirectionAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *NNISessionDirection) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into NNISessionDirectionAnyOf
	err = json.Unmarshal(data, &dst.NNISessionDirectionAnyOf);
	if err == nil {
		jsonNNISessionDirectionAnyOf, _ := json.Marshal(dst.NNISessionDirectionAnyOf)
		if string(jsonNNISessionDirectionAnyOf) == "{}" { // empty struct
			dst.NNISessionDirectionAnyOf = nil
		} else {
			return nil // data stored in dst.NNISessionDirectionAnyOf, return on the first match
		}
	} else {
		dst.NNISessionDirectionAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(NNISessionDirection)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *NNISessionDirection) MarshalJSON() ([]byte, error) {
	if src.NNISessionDirectionAnyOf != nil {
		return json.Marshal(&src.NNISessionDirectionAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableNNISessionDirection struct {
	value *NNISessionDirection
	isSet bool
}

func (v NullableNNISessionDirection) Get() *NNISessionDirection {
	return v.value
}

func (v *NullableNNISessionDirection) Set(val *NNISessionDirection) {
	v.value = val
	v.isSet = true
}

func (v NullableNNISessionDirection) IsSet() bool {
	return v.isSet
}

func (v *NullableNNISessionDirection) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNNISessionDirection(val *NNISessionDirection) *NullableNNISessionDirection {
	return &NullableNNISessionDirection{value: val, isSet: true}
}

func (v NullableNNISessionDirection) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNNISessionDirection) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


