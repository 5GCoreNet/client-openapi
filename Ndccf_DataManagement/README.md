# Go API client for Ndccf_DataManagement
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_DataManagement.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_DataManagement.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ndccf_DataManagement.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ndccf_DataManagement.ContextOperationServerVariables, map[string]map[string]string{
