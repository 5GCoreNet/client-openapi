# Go API client for Nrouter_SMService
...
```golang
ctx := context.WithValue(context.Background(), Nrouter_SMService.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nrouter_SMService.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nrouter_SMService.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nrouter_SMService.ContextOperationServerVariables, map[string]map[string]string{
