...
func (dst *NotificationType1) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(NotificationType1)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(NotificationType1)")
	}
}
...
func NewNullableNotificationType1(val *NotificationType1) *NullableNotificationType1 {
	return &NullableNotificationType1{value: val, isSet: true}
}
