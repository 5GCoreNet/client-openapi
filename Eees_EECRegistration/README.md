# Go API client for openapi_Eees_EECRegistration
...
```golang
import openapi_Eees_EECRegistration "///openapi_Eees_EECRegistration"
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Eees_EECRegistration.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Eees_EECRegistration.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Eees_EECRegistration.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), openapi_Eees_EECRegistration.ContextOperationServerVariables, map[string]map[string]string{
