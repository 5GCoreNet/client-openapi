# Go API client for MSGG_L3GDelivery
...
```golang
ctx := context.WithValue(context.Background(), MSGG_L3GDelivery.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), MSGG_L3GDelivery.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), MSGG_L3GDelivery.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), MSGG_L3GDelivery.ContextOperationServerVariables, map[string]map[string]string{
