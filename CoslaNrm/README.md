...
```golang
ctx := context.WithValue(context.Background(), CoslaNrm.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), CoslaNrm.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), CoslaNrm.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), CoslaNrm.ContextOperationServerVariables, map[string]map[string]string{
