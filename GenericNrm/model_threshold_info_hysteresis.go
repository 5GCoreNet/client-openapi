...
func Float32AsThresholdInfoHysteresis(v *float32) ThresholdInfoHysteresis {
	return ThresholdInfoHysteresis{
		Float32: v,
	}
}
...
func (dst *ThresholdInfoHysteresis) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(ThresholdInfoHysteresis)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(ThresholdInfoHysteresis)")
	}
}
...
func NewNullableThresholdInfoHysteresis(val *ThresholdInfoHysteresis) *NullableThresholdInfoHysteresis {
	return &NullableThresholdInfoHysteresis{value: val, isSet: true}
}
