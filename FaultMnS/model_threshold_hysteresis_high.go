...
func Float32AsThresholdHysteresisHigh(v *float32) ThresholdHysteresisHigh {
	return ThresholdHysteresisHigh{
		Float32: v,
	}
}
...
func (dst *ThresholdHysteresisHigh) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(ThresholdHysteresisHigh)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(ThresholdHysteresisHigh)")
	}
}
...
func NewNullableThresholdHysteresisHigh(val *ThresholdHysteresisHigh) *NullableThresholdHysteresisHigh {
	return &NullableThresholdHysteresisHigh{value: val, isSet: true}
}
