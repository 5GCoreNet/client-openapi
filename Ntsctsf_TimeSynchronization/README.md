# Go API client for Ntsctsf_TimeSynchronization
...
```golang
import Ntsctsf_TimeSynchronization "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_TimeSynchronization.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_TimeSynchronization.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_TimeSynchronization.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ntsctsf_TimeSynchronization.ContextOperationServerVariables, map[string]map[string]string{
