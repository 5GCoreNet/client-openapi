# Go API client for StreamingDataMnS
...
```golang
ctx := context.WithValue(context.Background(), StreamingDataMnS.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), StreamingDataMnS.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), StreamingDataMnS.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), StreamingDataMnS.ContextOperationServerVariables, map[string]map[string]string{
