# Go API client for MBSUserDataIngestSession
...
```golang
import MBSUserDataIngestSession "//"
```
...
```golang
ctx := context.WithValue(context.Background(), MBSUserDataIngestSession.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), MBSUserDataIngestSession.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), MBSUserDataIngestSession.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), MBSUserDataIngestSession.ContextOperationServerVariables, map[string]map[string]string{
