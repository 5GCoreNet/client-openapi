...
func (a *IPTVConfigurationDataStoreApiService) ReadIPTVCongifurationDataExecute(r ApiReadIPTVCongifurationDataRequest) ([]IptvConfigData, *http.Response, error) {
	...
	if r.configIds != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("config-ids", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("config-ids", parameterToString(t, "multi"))
		}
	}
	if r.dnns != nil {
		t := *r.dnns
	...
	if r.interGroupIds != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("inter-group-ids", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("inter-group-ids", parameterToString(t, "multi"))
		}
	}
