# Go API client for VAE_HDMapDynamicInfo
...
```golang
ctx := context.WithValue(context.Background(), VAE_HDMapDynamicInfo.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), VAE_HDMapDynamicInfo.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), VAE_HDMapDynamicInfo.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), VAE_HDMapDynamicInfo.ContextOperationServerVariables, map[string]map[string]string{
