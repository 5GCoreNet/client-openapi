/*
Nhss_imsSDM

Nhss Subscriber Data Management Service for IMS.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Nhss_imsSDM

import (
	"encoding/json"
	"fmt"
)

// ImsRegistrationState Represents the IMS status of the user or Public Identity or the IRS
type ImsRegistrationState struct {
	ImsRegistrationStateAnyOf *ImsRegistrationStateAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *ImsRegistrationState) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into ImsRegistrationStateAnyOf
	err = json.Unmarshal(data, &dst.ImsRegistrationStateAnyOf);
	if err == nil {
		jsonImsRegistrationStateAnyOf, _ := json.Marshal(dst.ImsRegistrationStateAnyOf)
		if string(jsonImsRegistrationStateAnyOf) == "{}" { // empty struct
			dst.ImsRegistrationStateAnyOf = nil
		} else {
			return nil // data stored in dst.ImsRegistrationStateAnyOf, return on the first match
		}
	} else {
		dst.ImsRegistrationStateAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(ImsRegistrationState)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *ImsRegistrationState) MarshalJSON() ([]byte, error) {
	if src.ImsRegistrationStateAnyOf != nil {
		return json.Marshal(&src.ImsRegistrationStateAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableImsRegistrationState struct {
	value *ImsRegistrationState
	isSet bool
}

func (v NullableImsRegistrationState) Get() *ImsRegistrationState {
	return v.value
}

func (v *NullableImsRegistrationState) Set(val *ImsRegistrationState) {
	v.value = val
	v.isSet = true
}

func (v NullableImsRegistrationState) IsSet() bool {
	return v.isSet
}

func (v *NullableImsRegistrationState) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableImsRegistrationState(val *ImsRegistrationState) *NullableImsRegistrationState {
	return &NullableImsRegistrationState{value: val, isSet: true}
}

func (v NullableImsRegistrationState) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableImsRegistrationState) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


