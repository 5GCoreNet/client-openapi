...
func Interface{}AsImeiSvInformation(v *interface{}) ImeiSvInformation {
...
func (dst *ImeiSvInformation) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(ImeiSvInformation)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(ImeiSvInformation)")
	}
}
...
func NewNullableImeiSvInformation(val *ImeiSvInformation) *NullableImeiSvInformation {
	return &NullableImeiSvInformation{value: val, isSet: true}
}
