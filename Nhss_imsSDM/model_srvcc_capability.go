/*
Nhss_imsSDM

Nhss Subscriber Data Management Service for IMS.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.1.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Nhss_imsSDM

import (
	"encoding/json"
	"fmt"
)

// SrvccCapability Represents the type of SRVCC support of the UE
type SrvccCapability struct {
	SrvccCapabilityAnyOf *SrvccCapabilityAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *SrvccCapability) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into SrvccCapabilityAnyOf
	err = json.Unmarshal(data, &dst.SrvccCapabilityAnyOf);
	if err == nil {
		jsonSrvccCapabilityAnyOf, _ := json.Marshal(dst.SrvccCapabilityAnyOf)
		if string(jsonSrvccCapabilityAnyOf) == "{}" { // empty struct
			dst.SrvccCapabilityAnyOf = nil
		} else {
			return nil // data stored in dst.SrvccCapabilityAnyOf, return on the first match
		}
	} else {
		dst.SrvccCapabilityAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(SrvccCapability)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *SrvccCapability) MarshalJSON() ([]byte, error) {
	if src.SrvccCapabilityAnyOf != nil {
		return json.Marshal(&src.SrvccCapabilityAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableSrvccCapability struct {
	value *SrvccCapability
	isSet bool
}

func (v NullableSrvccCapability) Get() *SrvccCapability {
	return v.value
}

func (v *NullableSrvccCapability) Set(val *SrvccCapability) {
	v.value = val
	v.isSet = true
}

func (v NullableSrvccCapability) IsSet() bool {
	return v.isSet
}

func (v *NullableSrvccCapability) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSrvccCapability(val *SrvccCapability) *NullableSrvccCapability {
	return &NullableSrvccCapability{value: val, isSet: true}
}

func (v NullableSrvccCapability) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSrvccCapability) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


