# Go API client for M5_ConsumptionReporting
...
```golang
import M5_ConsumptionReporting "//"
```
...
```golang
ctx := context.WithValue(context.Background(), M5_ConsumptionReporting.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), M5_ConsumptionReporting.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), M5_ConsumptionReporting.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), M5_ConsumptionReporting.ContextOperationServerVariables, map[string]map[string]string{
