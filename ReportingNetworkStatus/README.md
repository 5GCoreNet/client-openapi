# Go API client for ReportingNetworkStatus
...
```golang
ctx := context.WithValue(context.Background(), ReportingNetworkStatus.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), ReportingNetworkStatus.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), ReportingNetworkStatus.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), ReportingNetworkStatus.ContextOperationServerVariables, map[string]map[string]string{
