# Go API client for Nipsmgw_SMService
...
```golang
ctx := context.WithValue(context.Background(), Nipsmgw_SMService.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nipsmgw_SMService.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nipsmgw_SMService.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nipsmgw_SMService.ContextOperationServerVariables, map[string]map[string]string{
