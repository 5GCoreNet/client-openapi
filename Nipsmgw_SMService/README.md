# Go API client for openapi_Nipsmgw_SMService
...
```golang
import openapi_Nipsmgw_SMService "///openapi_Nipsmgw_SMService"
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Nipsmgw_SMService.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Nipsmgw_SMService.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), openapi_Nipsmgw_SMService.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), openapi_Nipsmgw_SMService.ContextOperationServerVariables, map[string]map[string]string{
