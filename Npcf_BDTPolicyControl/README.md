# Go API client for Npcf_BDTPolicyControl
...
```golang
ctx := context.WithValue(context.Background(), Npcf_BDTPolicyControl.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_BDTPolicyControl.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_BDTPolicyControl.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Npcf_BDTPolicyControl.ContextOperationServerVariables, map[string]map[string]string{
