# Go API client for Eecs_EESRegistration
...
```golang
ctx := context.WithValue(context.Background(), Eecs_EESRegistration.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Eecs_EESRegistration.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Eecs_EESRegistration.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Eecs_EESRegistration.ContextOperationServerVariables, map[string]map[string]string{
