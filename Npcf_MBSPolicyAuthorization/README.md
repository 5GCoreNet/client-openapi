# Go API client for Npcf_MBSPolicyAuthorization
...
```golang
import Npcf_MBSPolicyAuthorization "//"
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_MBSPolicyAuthorization.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_MBSPolicyAuthorization.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Npcf_MBSPolicyAuthorization.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Npcf_MBSPolicyAuthorization.ContextOperationServerVariables, map[string]map[string]string{
