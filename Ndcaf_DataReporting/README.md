# Go API client for Ndcaf_DataReporting
...
```golang
ctx := context.WithValue(context.Background(), Ndcaf_DataReporting.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ndcaf_DataReporting.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ndcaf_DataReporting.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ndcaf_DataReporting.ContextOperationServerVariables, map[string]map[string]string{
