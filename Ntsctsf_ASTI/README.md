# Go API client for Ntsctsf_ASTI
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_ASTI.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_ASTI.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Ntsctsf_ASTI.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Ntsctsf_ASTI.ContextOperationServerVariables, map[string]map[string]string{
