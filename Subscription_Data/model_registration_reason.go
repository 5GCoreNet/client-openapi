/*
Unified Data Repository Service API file for subscription data

Unified Data Repository Service (subscription data).   The API version is defined in 3GPP TS 29.504.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: -
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Subscription_Data

import (
	"encoding/json"
	"fmt"
)

// RegistrationReason struct for RegistrationReason
type RegistrationReason struct {
	RegistrationReasonAnyOf *RegistrationReasonAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *RegistrationReason) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into RegistrationReasonAnyOf
	err = json.Unmarshal(data, &dst.RegistrationReasonAnyOf);
	if err == nil {
		jsonRegistrationReasonAnyOf, _ := json.Marshal(dst.RegistrationReasonAnyOf)
		if string(jsonRegistrationReasonAnyOf) == "{}" { // empty struct
			dst.RegistrationReasonAnyOf = nil
		} else {
			return nil // data stored in dst.RegistrationReasonAnyOf, return on the first match
		}
	} else {
		dst.RegistrationReasonAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(RegistrationReason)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *RegistrationReason) MarshalJSON() ([]byte, error) {
	if src.RegistrationReasonAnyOf != nil {
		return json.Marshal(&src.RegistrationReasonAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableRegistrationReason struct {
	value *RegistrationReason
	isSet bool
}

func (v NullableRegistrationReason) Get() *RegistrationReason {
	return v.value
}

func (v *NullableRegistrationReason) Set(val *RegistrationReason) {
	v.value = val
	v.isSet = true
}

func (v NullableRegistrationReason) IsSet() bool {
	return v.isSet
}

func (v *NullableRegistrationReason) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRegistrationReason(val *RegistrationReason) *NullableRegistrationReason {
	return &NullableRegistrationReason{value: val, isSet: true}
}

func (v NullableRegistrationReason) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRegistrationReason) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


