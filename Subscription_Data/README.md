# Go API client for Subscription_Data
...
```golang
ctx := context.WithValue(context.Background(), Subscription_Data.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Subscription_Data.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Subscription_Data.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Subscription_Data.ContextOperationServerVariables, map[string]map[string]string{
...
*ProvisionedParameterDataEntryDocumentApi* | [**DeletePPDataEntry**](docs/ProvisionedParameterDataEntryDocumentApi.md#deleteppdataentry) | **Delete** /subscription-data/{ueId}/pp-data-store/{afInstanceId} | Delete a Provisioning Parameter Data Entry
