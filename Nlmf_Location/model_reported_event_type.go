/*
LMF Location

LMF Location Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.2.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package Nlmf_Location

import (
	"encoding/json"
	"fmt"
)

// ReportedEventType Specifies type of event reporting.
type ReportedEventType struct {
	ReportedEventTypeAnyOf *ReportedEventTypeAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *ReportedEventType) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into ReportedEventTypeAnyOf
	err = json.Unmarshal(data, &dst.ReportedEventTypeAnyOf);
	if err == nil {
		jsonReportedEventTypeAnyOf, _ := json.Marshal(dst.ReportedEventTypeAnyOf)
		if string(jsonReportedEventTypeAnyOf) == "{}" { // empty struct
			dst.ReportedEventTypeAnyOf = nil
		} else {
			return nil // data stored in dst.ReportedEventTypeAnyOf, return on the first match
		}
	} else {
		dst.ReportedEventTypeAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(ReportedEventType)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *ReportedEventType) MarshalJSON() ([]byte, error) {
	if src.ReportedEventTypeAnyOf != nil {
		return json.Marshal(&src.ReportedEventTypeAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableReportedEventType struct {
	value *ReportedEventType
	isSet bool
}

func (v NullableReportedEventType) Get() *ReportedEventType {
	return v.value
}

func (v *NullableReportedEventType) Set(val *ReportedEventType) {
	v.value = val
	v.isSet = true
}

func (v NullableReportedEventType) IsSet() bool {
	return v.isSet
}

func (v *NullableReportedEventType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReportedEventType(val *ReportedEventType) *NullableReportedEventType {
	return &NullableReportedEventType{value: val, isSet: true}
}

func (v NullableReportedEventType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReportedEventType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


