/*
Nsmf_PDUSession

SMF PDU Session Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.3.0-alpha.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Nsmf_PDUSession

import (
	"encoding/json"
	"fmt"
)

// ProtectionResult Protection Result of the security policy indicated as \"preferred\". Possible values are   - PERFORMED   - NOT_PERFORMED 
type ProtectionResult struct {
	ProtectionResultAnyOf *ProtectionResultAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *ProtectionResult) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into ProtectionResultAnyOf
	err = json.Unmarshal(data, &dst.ProtectionResultAnyOf);
	if err == nil {
		jsonProtectionResultAnyOf, _ := json.Marshal(dst.ProtectionResultAnyOf)
		if string(jsonProtectionResultAnyOf) == "{}" { // empty struct
			dst.ProtectionResultAnyOf = nil
		} else {
			return nil // data stored in dst.ProtectionResultAnyOf, return on the first match
		}
	} else {
		dst.ProtectionResultAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(ProtectionResult)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *ProtectionResult) MarshalJSON() ([]byte, error) {
	if src.ProtectionResultAnyOf != nil {
		return json.Marshal(&src.ProtectionResultAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableProtectionResult struct {
	value *ProtectionResult
	isSet bool
}

func (v NullableProtectionResult) Get() *ProtectionResult {
	return v.value
}

func (v *NullableProtectionResult) Set(val *ProtectionResult) {
	v.value = val
	v.isSet = true
}

func (v NullableProtectionResult) IsSet() bool {
	return v.isSet
}

func (v *NullableProtectionResult) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProtectionResult(val *ProtectionResult) *NullableProtectionResult {
	return &NullableProtectionResult{value: val, isSet: true}
}

func (v NullableProtectionResult) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProtectionResult) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


