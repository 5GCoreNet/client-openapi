# Go API client for AnalyticsExposure
...
```golang
ctx := context.WithValue(context.Background(), AnalyticsExposure.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), AnalyticsExposure.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), AnalyticsExposure.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), AnalyticsExposure.ContextOperationServerVariables, map[string]map[string]string{
