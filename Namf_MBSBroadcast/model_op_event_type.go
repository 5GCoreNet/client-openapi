/*
Namf_MBSBroadcast

AMF MBSBroadcast Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 

API version: 1.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_Namf_MBSBroadcast

import (
	"encoding/json"
	"fmt"
)

// OpEventType Operation Event Type.
type OpEventType struct {
	OpEventTypeAnyOf *OpEventTypeAnyOf
	string *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *OpEventType) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into OpEventTypeAnyOf
	err = json.Unmarshal(data, &dst.OpEventTypeAnyOf);
	if err == nil {
		jsonOpEventTypeAnyOf, _ := json.Marshal(dst.OpEventTypeAnyOf)
		if string(jsonOpEventTypeAnyOf) == "{}" { // empty struct
			dst.OpEventTypeAnyOf = nil
		} else {
			return nil // data stored in dst.OpEventTypeAnyOf, return on the first match
		}
	} else {
		dst.OpEventTypeAnyOf = nil
	}

	// try to unmarshal JSON data into string
	err = json.Unmarshal(data, &dst.string);
	if err == nil {
		jsonstring, _ := json.Marshal(dst.string)
		if string(jsonstring) == "{}" { // empty struct
			dst.string = nil
		} else {
			return nil // data stored in dst.string, return on the first match
		}
	} else {
		dst.string = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(OpEventType)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src *OpEventType) MarshalJSON() ([]byte, error) {
	if src.OpEventTypeAnyOf != nil {
		return json.Marshal(&src.OpEventTypeAnyOf)
	}

	if src.string != nil {
		return json.Marshal(&src.string)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableOpEventType struct {
	value *OpEventType
	isSet bool
}

func (v NullableOpEventType) Get() *OpEventType {
	return v.value
}

func (v *NullableOpEventType) Set(val *OpEventType) {
	v.value = val
	v.isSet = true
}

func (v NullableOpEventType) IsSet() bool {
	return v.isSet
}

func (v *NullableOpEventType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOpEventType(val *OpEventType) *NullableOpEventType {
	return &NullableOpEventType{value: val, isSet: true}
}

func (v NullableOpEventType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOpEventType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


