# Go API client for CpProvisioning
...
```golang
ctx := context.WithValue(context.Background(), CpProvisioning.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), CpProvisioning.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), CpProvisioning.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), CpProvisioning.ContextOperationServerVariables, map[string]map[string]string{
