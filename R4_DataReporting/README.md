# Go API client for R4_DataReporting
...
```golang
ctx := context.WithValue(context.Background(), R4_DataReporting.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), R4_DataReporting.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), R4_DataReporting.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), R4_DataReporting.ContextOperationServerVariables, map[string]map[string]string{
