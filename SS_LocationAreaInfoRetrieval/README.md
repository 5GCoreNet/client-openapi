# Go API client for SS_LocationAreaInfoRetrieval
...
```golang
import SS_LocationAreaInfoRetrieval "//"
```
...
```golang
ctx := context.WithValue(context.Background(), SS_LocationAreaInfoRetrieval.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), SS_LocationAreaInfoRetrieval.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), SS_LocationAreaInfoRetrieval.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), SS_LocationAreaInfoRetrieval.ContextOperationServerVariables, map[string]map[string]string{
