# Go API client for UAE_RealtimeUAVStatus
...
```golang
ctx := context.WithValue(context.Background(), UAE_RealtimeUAVStatus.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), UAE_RealtimeUAVStatus.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), UAE_RealtimeUAVStatus.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), UAE_RealtimeUAVStatus.ContextOperationServerVariables, map[string]map[string]string{
