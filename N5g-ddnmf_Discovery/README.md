# Go API client for N5g-ddnmf_Discovery
...
```golang
ctx := context.WithValue(context.Background(), N5g-ddnmf_Discovery.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), N5g-ddnmf_Discovery.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), N5g-ddnmf_Discovery.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), N5g-ddnmf_Discovery.ContextOperationServerVariables, map[string]map[string]string{
