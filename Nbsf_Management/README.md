# Go API client for Nbsf_Management
...
```golang
ctx := context.WithValue(context.Background(), Nbsf_Management.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nbsf_Management.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nbsf_Management.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nbsf_Management.ContextOperationServerVariables, map[string]map[string]string{
