# Go API client for Nadrf_DataManagement
...
```golang
ctx := context.WithValue(context.Background(), Nadrf_DataManagement.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), Nadrf_DataManagement.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), Nadrf_DataManagement.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), Nadrf_DataManagement.ContextOperationServerVariables, map[string]map[string]string{
